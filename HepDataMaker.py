...
def add_hepdata_table(i_count, observable, min_pt, max_pt):
    ...
    f_final_name = "histograms.root"
    f_final_path = os.path.join("/home", "alicexty", "yasp", "software", "root", "6.28.12", "macros", "inputs", f_final_name)
    #f_final = ROOT.TFile(f_final_path, "READ")
    hepdata_reader = hepdata_lib.RootFileReader(f_final_path)

    # Define variables
    h_name = "unfold1d_%s_%i_%i" % (observable, min_pt, max_pt)
...
def set_hepdata_table_descriptors(table, observable, min_pt, max_pt):
    ...
    if observable == 'z':
      ...
      if np.isclose(min_pt, 30.):
        table.location = "Figure 1, Figure 4"
...
#----------------------------------------------------------------------
# Create submission
hepdata_dir = os.path.join("/home", "alicexty", "yasp", "software", "root", "6.28.12", "macros", "outputs")
